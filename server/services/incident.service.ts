		...
		if (!location) {
			...
			if (!location) {
				const error = new Error(
					"No se pudieron determinar las coordenadas de la direccion, por favor introduce una valida"
				);
...
export const getAllIncidents = async (): Promise<Array<Incident> | never> => {
	try {
		...
		const incidents: Array<Incident> = await repository
