...
export const userDetailController = async (req: any, res: any) => {
	const userEmail: string | undefined = req.query.userEmail;
...
export const userCreateController = async (req: any, res: any) => {
	...
	try {
		...
		const result = await createPGUser({
			email: newUser.email,
			uid: newUser.uid,
		});
...
export const userUpdateController = async (req: any, res: any) => {
	try {
		...
		const updatedUser: admin.auth.UserRecord = await admin
			.auth()
			.updateUser(user.uid, req.body);
		res.json(
			AuthResponse({
